...
if($_POST){
    ...
    if(get_query_val('fn_upmark', 'count(id)', array('roomid' => $_SESSION['roomid'], 'userid' => $_SESSION['userid'], 'status' => '未处理', 'type' => '下分'))){
        ajaxMsg('您还有一笔下分请求等待处理',0);
    }
    $res = withdrawal($_SESSION['username'], $_SESSION['userid'], $money);
    if($res > 0){
        ajaxMsg('下分请求提交成功');
    }elseif($res == -1){
        ajaxMsg('您的账户余额不足，无法下分' , 0);
    }else{
        ajaxMsg('下分请求提交失败',0);
    }
}
    ...
    <div class="wx_cfb_account_center_wrap">
        ...
        <div class="wx_cfb_ac_fund_detail">
            <div class="user_info clearfix">
                ...
                <div class="titlexb">下分</div>
            </div>
        </div>
        <ul id="recharge_list">
            <li>线上下分</li>
            <li class="active">线下下分</li>
        </ul>
        <div style="padding: 20px;padding-top: 0px;">
            ...
            <div id="offline_pay" style="padding-top: 20px;">
                <p>下分金额</p>
                <input type="text" placeholder="请输入下分金额" class="recharge_money" name="money" />
                ...
                <dl class="room_info">
                    <dd class="tip_title">收款信息</dd>
                    <dt id="weixin_panel">
                        <img class="image" src="<?php echo $user['weichat_image'];?>" />
                        <p>收款微信：<?php echo $user['weichat_number'];?> </p>
                        <p>收款人姓名：<?php echo $user['weichat_name'];?> </p>
                    </dt>
                    <dt id="alipay_panel" style="display: none">
                        <img class="image" src="<?php echo $user['alipay_image'];?>" />
                        <p>收款微信：<?php echo $user['alipay_number'];?> </p>
                        <p>收款人姓名：<?php echo $user['alipay_name'];?> </p>
                    </dt>
                    <dt id="alipay_panelsssss" style="display: none;">
                    ...
                    <dt id="union_panel" style="display: none">
                        <p>银行名称：<?php echo $user['bank_name'];?> </p>
                        <p>分行名称：<?php echo $user['bank_fenhang'];?> </p>
                        <p>收款人姓名：<?php echo $user['bank_username'];?> </p>
                        <p>银行卡号：<?php echo $user['bank_number'];?> </p>
                    </dt>
                </dl>
...
<script type="text/javascript">
    ...
    $(function (){
        ...
        $('#submit').on('click' , function (){
            ...
            if(money == ''){
                return jqtoast('请输入下分金额');
            }
            money = parseFloat(money);
            if(isNaN(money)){
                return jqtoast('下分金额输入错误');
            }
            $.ajax({
                url:'',
                data:{money:money},
