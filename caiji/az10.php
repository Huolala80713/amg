...
function caiji(){
    writeLog("澳洲幸运10开始爬取数据！" . time(),'az10');
    $typeid = '1';
    ...
    if((!$topcode || $topcode[0] <> $qihao) && $qihao){
        $id = '';
        insert_query('fn_open', array('term' => $qihao, 'code' => $opencode, 'time' => date('Y-m-d H:i:s',strtotime($opentime)), 'type' => $typeid, 'next_term' => $next_term, 'next_time' => date('Y-m-d H:i:s',$nexttime)) , $id);
...
//    kaichat($typeid, $next_term , $qihao , $opencode);
        writeLog("更新 $qihao 成功！" . time(),'az10');
    }else{
        writeLog("等待 $next_term 刷新！" . time(),'az10');
    }
    writeLog("澳洲幸运10爬取数据结束！" . time(),'az10');
}