...
if($type == '1'){
    ...
    $id = $_POST['id'];
    switch($game){
    case 'xy28': $table = "fn_order";
        break;
    case "jnd28": $table = "fn_order";
        break;
    case "jsmt": $table = "fn_order";
        break;
    case "jssc": $table = "fn_jsscorder";
        break;
    case "jsssc": $table = "fn_jssscorder";
        break;
    case "cqssc": $table = "fn_order";
        break;
    ...
    backmoney($userid, $m, $term, $_SESSION['agent_room'],$game);
    echo json_encode(array("success" => true));
}elseif($type == '3'){
    $game = $_POST['game'];
    if($game == 'pk10'){
        ...
        delete_query('fn_order', "roomid = '{$_SESSION['agent_room']}' and status = '未结算' and `term` < 2000000");
    }elseif($game == 'jnd28'){
        delete_query('fn_order', "roomid = '{$_SESSION['agent_room']}' and status = '未结算' and `term` > 2000000");
    }elseif($game == 'cqssc'){
        delete_query('fn_order', "roomid = '{$_SESSION['agent_room']}' and status = '未结算'");
    }elseif($game == 'jsssc'){
    ...
    if($game == 'pk10'){
        ...
        select_query('fn_order', '*', "roomid = '{$_SESSION['agent_room']}' and status = '未结算'");
        while($con = db_fetch_array()){
            $cons[] = $con;
        }
        foreach($cons as $con){
        ...
        update_query("fn_order", array("status" => "已撤单"), "roomid = '{$_SESSION['agent_room']}' and status = '未结算'");
    }elseif($game == 'jnd28'){
        select_query('fn_order', '*', "roomid = '{$_SESSION['agent_room']}' and status = '未结算' and `term` > 2000000");
        while($con = db_fetch_array()){
            $cons[] = $con;
        }
        foreach($cons as $con){
...
function backmoney($userid, $money, $term, $room , $game = ''){
    update_query('fn_user', array('money' => '+=' . $money), array('userid' => $userid, 'roomid' => $room));
    insert_query("fn_marklog", array("roomid" => $room, 'userid' => $userid, 'type' => '撤单退还','game' => $game, 'content' => '管理员退还' . $term . '期下注', 'money' => $money, 'addtime' => 'now()'));
}
?>