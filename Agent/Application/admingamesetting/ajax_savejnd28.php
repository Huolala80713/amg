...
if($_GET['form'] == 'form1'){
    ...
    update_query("fn_admin_lottery5", array("fengtime" => $fengtime, 'gameopen' => $gameopen), array('roomid' => $_SESSION['agent_room']));
    echo '<script>alert("保存成功~"); window.location.href="/zb9n8rUvp0.php?m=admin_g_setting&g=5";</script>';
}elseif($_GET['form'] == 'form4'){
    $content = $_POST['customcont'];
    doLog('操作' . getGameTxtName($gameID) . '彩种规则');
    update_query("fn_admin_lottery5", array("rules" => $content), array('roomid' => $_SESSION['agent_room']));
    echo '<script>alert("保存成功~"); window.location.href="/zb9n8rUvp0.php?m=admin_g_setting&g=5";</script>';
}
?>