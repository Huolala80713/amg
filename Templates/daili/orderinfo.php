...
$list = orderlist($user_ids, $day1 , $day2 , $game_name , $log_type , $_GET['p']>0?$_GET['p']:1 , 10);
$order_list = $list['list'];
$game_list = getGameList();
$page = getPageList($list['count'] , 10 , '/Templates/daili/orderinfo.php');
?>
<!DOCTYPE html>
...
<header class="m-navbar" style="">
    ...
    <div class="navbar-center">
        <span class="navbar-title" style="">投注明细</span>
    </div>
...
<div class="total-log">
    <table class="total-table" style="font-size: 0.3rem;">
        ...
        if(!empty($order_list)):
            ...
            if($order['status'] > 0 && $order['status'] != '已取消' && $order['status'] != '未结算'){
                $allyk += $order['status'];
            }
        }
            ?>
...
<script type="text/javascript">
    ...
    function search(type){
        ...
        window.location.href="?day1="+day1+"&day2="+day2+"&type="+type+"&game_name=" + game_name + "&log_type=" + $('input[name="log_type"]:checked').val() + "&back=<?php echo $back;?>&userid=<?php $_GET['userid'];?>";
    }
